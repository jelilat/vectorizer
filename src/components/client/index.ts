import { GRPC_URL } from '@/components/config';
...
export const getBuckets = async (rpcUrl: string) => {
...
export const getObjects = async (rpcUrl: string, bucketName: string) => {
  const rpcClient = await makeRpcClient(rpcUrl);

  const rpc = new storageQueryClientImpl(rpcClient);
  const res = await rpc.ListObjects({
    bucketName,
    pagination: undefined,
  });
  return res;
